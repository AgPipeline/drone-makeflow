...
EXPECTED_NUM_FOLDERS=8
EXPECTED_NUM_CANOPYCOVER_CSV=7
...
CHECK_FILE="${TARGET_FOLDER}/orthomosaic.tif"
if [[ -f "${CHECK_FILE}" ]]; then
  echo "Orthomosaic file found: ${CHECK_FILE}"
else
  echo "Unable to find orthomosaic file: ${CHECK_FILE}";
  exit 1;
fi
